...
time_h = time;
load('wrf_d1_RCP85_20062099.mat');
load('lat_d1_wrf.mat'); 
load('lon_d1_wrf.mat'); 
...
%     text(0.49,0.037,'70° W','FontSize',8,'FontWeight','normal','Units','normalized');
    ...
    saveas(hFig_win, 'EP_change_D1_win_R1.png'); 
...
%Setting lat/lon limits
   ...
   axes(ha(1));
    ...
    htt=title(['Difference in average annual - warm season'],'FontWeight','normal');
    %htt.Position = [htt.Position(1) htt.Position(2)-.024 htt.Position(3)];
    set(gca,'FontSize',18);
   
    axis off
    
    saveas(hFig_sum, 'EP_change_D1_sum_R1.png'); 
