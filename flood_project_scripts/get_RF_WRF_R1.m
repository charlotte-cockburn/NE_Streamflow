...
if(contains(river,'White'))
    ...
    load('WR_snowdepth_avg_WRFfut_test.mat');
    fut_data = [WR_future_data, table(regressed_snow_WRFfut_avg)];
...
elseif(contains(river,'Shenandoah'))
    ...
    load('SR_snowdepth_avg_WRFfut_test.mat');
    fut_data = [SR_future_data, table(regressed_snow_WRFfut_avg)];    
    acronym = 'SR';
...
elseif(contains(river,'Mattawamkeag'))
    ...
    load('MR_snowdepth_avg_WRFfut_test.mat');
    fut_data = [MR_future_data, table(regressed_snow_WRFfut_avg)];
    acronym = 'MR';
...
elseif(contains(river, 'Diamond'))
    ...
    load('DR_snowdepth_avg_WRFfut_test.mat');
    fut_data = [DR_future_data, table(regressed_snow_WRFfut_avg)];
    acronym = 'DR';
...
if(contains(river,'White'))
    ...
    save('RF_WRF_fut_WR_R1.mat', 'RF_WRF_fut_WR');
elseif(contains(river,'Shenandoah'))
    RF_WRF_fut_SR = RF_data_table;
    save('RF_WRF_fut_SR_R1.mat', 'RF_WRF_fut_SR');
elseif(contains(river,'Mattawamkeag'))
    RF_WRF_fut_MR = RF_data_table;
    save('RF_WRF_fut_MR_R1.mat', 'RF_WRF_fut_MR');
elseif(contains(river, 'Diamond'))
    RF_WRF_fut_DR = RF_data_table;
    save('RF_WRF_fut_DR_R1.mat', 'RF_WRF_fut_DR');
end
