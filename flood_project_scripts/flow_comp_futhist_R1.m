...
% Change discharge from cfs to m3s
...
performance = [rmse_std_all_win.*100, rmse_std_all_win_ws.*100, rmse_std_all_sum.*100;...
    corr_all_win(2)^2, corr_all_win_ws(2)^2, corr_all_sum(2)^2];
performance_99 = [rmse_std_99_win.*100, rmse_std_99_win_ws.*100, rmse_std_99_sum.*100; ...
    corr_99_win_ws(2)^2, corr_99_win_ws(2)^2, corr_99_sum(2)^2]; 
...
rf_preds_win = oobPredict(RF_model_winter); 
first_date = datetime(1976,1,1); 
last_date = datetime(2005,12,31); 
dates = datetime(RF_table.Year, RF_table.Month, RF_table.Day); 
dates_win = dates(winter_indices);
yes_dates_win = false(length(dates_win)); 
yes_dates_win(dates_win>first_date & dates_win < last_date) = true; 
yes_dates_win(month(dates_win)==2 & day(dates_win)==29)= false; 
rf_preds_win = rf_preds_win(yes_dates_win);

bias_win = (sum(WRF_preds_win)-sum(rf_preds_win))/sum(rf_preds_win);
bias_win = round(bias_win,4)*100; 

rf_preds_win_noSnow = oobPredict(RF_model_winter_NoSnow); 
rf_preds_win_noSnow = rf_preds_win_noSnow(yes_dates_win); 
bias_win_noSnow = (sum(WRF_preds_win_ws)-sum(rf_preds_win_noSnow))/sum(rf_preds_win_noSnow);
bias_win_noSnow = round(bias_win_noSnow,4)*100;

rf_preds_sum = oobPredict(RF_model_summer); 
dates_sum = dates(summer_indices);
yes_dates_sum = false(length(dates_sum)); 
yes_dates_sum(dates_sum>first_date & dates_sum < last_date) = true; 
yes_dates_win(month(dates_sum)==2 & day(dates_sum)==29)= false; 
rf_preds_sum = rf_preds_sum(yes_dates_sum);

bias_sum = (sum(WRF_preds_sum)-sum(rf_preds_sum))/sum(rf_preds_sum);
bias_sum = round(bias_sum,4)*100;

biases = [bias_win, bias_win_noSnow, bias_sum]; 
...
top_future_win_ws = WRF_preds_fut_win_ws(WRF_preds_fut_win_ws>=threshold_win_ws);
...
perc_change_win_ws = (win_fut_ws-win_hist_ws)/win_hist_ws;
...
perc_change_all_win_ws = (all_win_fut_ws-all_win_hist_ws)/all_win_hist_ws;
...
% histogram(WRF_preds_win, 'BinWidth', 500); 
% hold on;
% histogram(WRF_preds_fut_win, 'BinWidth', 500); 
% leg = legend('Historical', 'Future', 'FontSize', 16) ;
% xlabel('Discharge [cfs]', 'FontSize', 16); 
% ylabel('Frequency', 'FontSize', 16);
% if(perc_change_all_win>=0)
%     title(strcat(title_name, ' (+',num2str(perc_change_all_win), '%)'), 'FontSize', 22);
% else
%     title(strcat(title_name, ' (',num2str(perc_change_all_win), '%)'), 'FontSize', 22);
% end
% 
% axes('Position',[leg.Position(1) 0.4 leg.Position(3) 0.3]);
% histogram(top_historical_win, 'BinWidth', 500); 
% hold on;
% histogram(top_future_win, 'BinWidth', 500); 
% if(perc_change_win>=0)
%     title(strcat('99th percentile flows (+',num2str(perc_change_win), '%)'), ...
%         'FontSize', 14);
% else
%     title(strcat('99th percentile flows (',num2str(perc_change_win), '%)'), ...
%         'FontSize', 14);
% end
% 
% %saveas(flows_fig_cs, string(strcat(acronym, '_flowchange_cs.png')));
% 
% %%
% flows_fig_ws = figure(3), clf ;
% set(flows_fig_ws, 'Position', [200 200 500 400]);
% histogram(WRF_preds_sum, 'BinWidth', 400); 
% hold on;
% histogram(WRF_preds_fut_sum, 'BinWidth', 400); 
% 
% leg2 = legend('Historical (1976-2005)', 'Future (2070-2099)', 'FontSize', 16) ;
% if(perc_change_all_sum>=0)
%     title(strcat(title_name, ' (+',num2str(perc_change_all_sum), '%)'), 'FontSize', 22);
% else
%     title(strcat(title_name, ' (',num2str(perc_change_all_sum), '%)'), 'FontSize', 22);
% end
% xlabel('Discharge [cfs]', 'FontSize', 16); 
% ylabel('Frequency', 'FontSize', 16); 
% 
% axes('Position',[leg2.Position(1) 0.4 leg2.Position(3) 0.3]);
% histogram(top_historical_sum, 'BinWidth', 200); 
% hold on;
% histogram(top_future_sum, 'BinWidth', 200); 
% if(perc_change_sum>=0)
%     title(strcat('99th percentile flows (+',num2str(perc_change_sum), '%)'), ...
%         'FontSize', 14);
% else
%     title(strcat('99th percentile flows (',num2str(perc_change_sum), '%)'),...
%         'FontSize', 14);
% end
% 
% %saveas(flows_fig_ws, string(strcat(acronym, '_flowchange_ws.png')));
...
saveas(flows_fig_cs, string(strcat(acronym, '_log_flowchange_cs_R1.tif')));
...
saveas(flows_fig_ws, string(strcat(acronym, '_log_flowchange_ws_R1.tif')));
...
saveas(flows_fig_cs_ws, string(strcat(acronym, '_log_flowchange_cs_noSnowEither.png')));
...
saveas(seasonal_fig, strcat(acronym, '_fut_hist_seasonal_cycles_R1.png')); 
